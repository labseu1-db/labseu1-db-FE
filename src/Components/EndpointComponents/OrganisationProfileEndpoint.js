  ...
  render() {
    return (
      <div>
        ...
        {this.props.organisation.arrayOfUsers && <div>arrayOfUsers: {this.props.organisation.arrayOfUsers.map((u, index) => <div key={index}>userEmail: {u.userEmail} userId: {u.userId}</div>
        )}</div>}
        {this.props.organisation.arrayOfAdmins && <div>arrayOfAdmins: {this.props.organisation.arrayOfAdmins.map((u, index) => <div key={index}>userEmail: {u.userEmail} userId: {u.userId}</div>
        )}</div>}
